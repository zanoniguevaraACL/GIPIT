...
import { handleBackToInterview } from "@/app/actions/handleBackToInterview";
...
function Page({ params }: { params: { processId: string; candidateId: string } }) {
  ...
  const fields: FormInputsRow = [
    [
      ...
      { type: "submit", value: "Regresar a entrevistas" },
    ],
  ];
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try{
      const result = await handleBackToInterview(formData, actualRoute);
